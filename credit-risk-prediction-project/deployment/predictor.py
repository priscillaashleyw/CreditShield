...
class CreditRiskPredictor:
    ...
    def load_artifacts(self):
        ...
        try:
            ...
            # Load FICO estimator (optional — degrades gracefully if missing)
            fico_path = self.model_dir / "fico_estimator.pkl"
            if fico_path.exists():
                self.fico_estimator = joblib.load(fico_path)
                mae = self.fico_estimator.get('mae', '?')
                print(f"✅ Loaded FICO estimator (MAE: {mae:.1f} pts)" if isinstance(mae, float) else "✅ Loaded FICO estimator")
            else:
                print("⚠️ fico_estimator.pkl not found — FICO must be provided manually")
    ...
    def preprocess_input(self, input_dict):
        ...
        if self.imputer is not None and not processed_df.empty:
            try:
                # Reorder columns to match the order the imputer was fitted with
                fitted_order = list(getattr(self.imputer, "feature_names_in_", processed_df.columns))
                processed_df = processed_df.reindex(columns=fitted_order, fill_value=0)
                processed_df = pd.DataFrame(
                    self.imputer.transform(processed_df),
                    columns=fitted_order
                )
            except Exception as e:
        ...
        if self.scaler is not None and not processed_df.empty:
            try:
                fitted_order = list(getattr(self.scaler, "feature_names_in_", processed_df.columns))
                processed_df = processed_df.reindex(columns=fitted_order, fill_value=0)
                processed_df = pd.DataFrame(
    ...
    def predict(self, input_dict):
        try:
            # --- FICO estimation (when user didn't provide credit score) ---
            fico_estimated = False
            fico_estimated_value = None

            fico_missing = (
                input_dict.get('last_fico_range_low') is None
                or input_dict.get('last_fico_range_low') == 0
            )
            if fico_missing and self.fico_estimator is not None:
                from sys import path as _path
                import os as _os
                _training_src = str(self.training_dir / "src")
                if _training_src not in _path:
                    _path.insert(0, _training_src)
                from fico_estimator import estimate_fico

                est_low, est_high = estimate_fico(self.fico_estimator, input_dict)
                input_dict = dict(input_dict)   # don't mutate the caller's dict
                input_dict['last_fico_range_low']  = est_low
                input_dict['last_fico_range_high'] = est_high
                fico_estimated = True
                fico_estimated_value = est_low
                print(f"📊 FICO estimated: {est_low} (±{self.fico_estimator.get('mae', '?'):.0f} pts)")
            ...
            return {
                ...
                "fico_estimated": fico_estimated,
                "fico_estimated_value": fico_estimated_value,
